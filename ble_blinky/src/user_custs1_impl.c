...
extern int load;
extern int new_load;
extern int load_array[LOAD_VALS_ARR_SIZE];
extern int num_vals;
 ...
 */
/*
void user_svc1_lc_val_wr_handler(ke_msg_id_t const msgid,
                          ...
                          ke_task_id_t const src_id)
{
    struct custs1_val_ntf_ind_req *req = KE_MSG_ALLOC_DYN(CUSTS1_VAL_NTF_REQ,
                                                          ...
                                                          DEF_SVC1_LC_VAL_CHAR_LEN);

    // LC value to be sampled
    //static uint16_t sample      __SECTION_ZERO("retention_mem_area0");
   // sample = (sample <= 0xffff) ? (sample + 1) : 0;

		//static uint32_t load_val      __SECTION_ZERO("retention_mem_area0");
		//load_val = load;
		
    //req->conhdl = app_env->conhdl;
    req->handle = SVC1_IDX_LC_VAL_VAL;
    req->length = 4*num_vals;//DEF_SVC1_LC_VAL_CHAR_LEN;
    req->notification = true;
    //memcpy(req->value, load, DEF_SVC1_LC_VAL_CHAR_LEN);
    memcpy(req->value, load_array, (4*num_vals));
...
void user_svc1_lc_val_rd_handler(ke_msg_id_t const msgid,
													struct custs1_val_write_ind const *param,
                          ke_task_id_t const dest_id,
                          ke_task_id_t const src_id)
{ 
...
/
/custs1_val_ntf_ind_req *req = KE_MSG_ALLOC_DYN(CUSTS1_VAL_NTF_REQ,
                                                          prf_get_task_from_id(TASK_ID_CUSTS1),
                                                          TASK_APP,
                                                          custs1_val_ntf_ind_req,
                                                          DEF_SVC1_LC_VAL_CHAR_LEN);
/
    ...
    req->length = 4*num_vals;//DEF_SVC1_LC_VAL_CHAR_LEN;
    //memcpy(req->value, load, DEF_SVC1_LC_VAL_CHAR_LEN);
    memcpy(req->value, load_array, (DEF_SVC1_LC_VAL_CHAR_LEN*num_vals));
...
void user_svc1_lc_val_rd_req_ind_handler(ke_msg_id_t const msgid,
																				 ...
																				 ke_task_id_t const src_id)
{
    ...
    //rsp->handle = SVC1_IDX_LC_VAL_VAL;
    rsp->length = DEF_SVC1_LC_VAL_CHAR_LEN * num_vals;
	  // Provide the ATT error code.
    rsp->status  = ATT_ERR_NO_ERROR;
	  
	  // Copy value
    memcpy(rsp->value, load_array, rsp->length);
    // Send message
    ke_msg_send(rsp);
...
void user_svc1_read_lc_val_handler(ke_msg_id_t const msgid,
                                   struct custs1_value_req_ind const *param,
                                   ke_task_id_t const dest_id,
                                   ke_task_id_t const src_id)
{
    ...
    rsp->length  = num_vals * DEF_SVC1_LC_VAL_CHAR_LEN;
    // Provide the ATT error code.
    rsp->status  = ATT_ERR_NO_ERROR;
    // Copy value
